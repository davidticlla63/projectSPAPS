...
@FacesConverter("tipoTareaConverter")
public class TipoTareaConverter implements Converter {
	...
	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		if (value != null && value.trim().length() > 0) {
			try {
				...
				List<TipoTarea> pacientes = new ArrayList<TipoTarea>();
				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("tarea.xhtml"))
					pacientes = TareaController.tipoTareas;
				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("desg_orden_servicio_listado.xhtml"))
					pacientes = DesgOrdenServicioAprobacionController.tipoTareas;
				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("gestion_tareas.xhtml"))
					pacientes = GestionTareaController.tipoTareas;
				TipoTarea paciente = getObjectFromList(pacientes,
	...
	public String getAsString(FacesContext fc, UIComponent uic, Object object) {
		if (object != null) {
			return String.valueOf(((TipoTarea) object).getId());
		} else {
			return null;
		}
	}
	...
	private TipoTarea getObjectFromList(final List<?> list,
			final Integer identifier) {
		for (final Object object : list) {
			final TipoTarea team = (TipoTarea) object;
