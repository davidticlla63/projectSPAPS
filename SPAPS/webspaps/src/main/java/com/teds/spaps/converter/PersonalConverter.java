...
public class PersonalConverter implements Converter {
	...
	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		if (value != null && value.trim().length() > 0) {
			try {
				...
				List<Personal> pacientes = new ArrayList<Personal>();
				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("desg_orden_servicio.xhtml"))
					pacientes = DesgOrdenServicioController.medicos;
				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("tarea.xhtml"))
					pacientes = TareaController.personales;
				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("gestion_tareas.xhtml"))
					pacientes = GestionTareaController.personales;

				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("index.xhtml")) {
					if (protocolo[protocolo.length - 2]
							.equals("ordenes_imagenologia")) {
						pacientes = LabOrdenImagenologiaController.listaPersonal;
					} else if (protocolo[protocolo.length - 2]
							.equals("ordenes")) {
						pacientes = LabOrdenController.listaPersonal;
					}
				}
				

				if (protocolo[protocolo.length - 1]
						.equalsIgnoreCase("listado_mis_tareas.xhtml"))
					pacientes = MisTareaController.personales;
				Personal paciente = getObjectFromList(pacientes,
	...
	public String getAsString(FacesContext fc, UIComponent uic, Object object) {
		if (object != null) {
			return String.valueOf(((Personal) object).getId());
		} else {
			return null;
		}
	}
	...
	private Personal getObjectFromList(final List<?> list,
			final Integer identifier) {
		for (final Object object : list) {
			final Personal team = (Personal) object;
