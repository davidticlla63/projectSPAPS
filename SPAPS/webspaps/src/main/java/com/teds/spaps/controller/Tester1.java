...
@ManagedBean(name = "tester1Controller")
@ApplicationScoped
public class Tester1 implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4678465332699681611L;
	...
	/*
	 * @SuppressWarnings({ "unused", "resource" }) public byte[] CargarFile() {
	 * try { File f = new File("D:/Imagen006.jpg"); // asociamos el archivo //
	 * fisico InputStream is = new FileInputStream(f); // lo abrimos. Lo //
	 * importante es que sea // un InputStream byte[] buffer = new byte[(int)
	 * f.length()]; // creamos el buffer int readers = is.read(buffer); return
	 * buffer; } catch (IOException e) { e.printStackTrace(); return null; } //
