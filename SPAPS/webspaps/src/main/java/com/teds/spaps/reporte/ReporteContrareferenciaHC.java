...
@WebServlet("/ReporteContrareferenciaHC")
public class ReporteContrareferenciaHC extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = -3180398475937509032L;
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			try {
				...
				String idContrareferencia = request
						.getParameter("idContrareferencia");
				// String pPathLogo = request.getParameter("pLogo");
				Contrareferencia contrareferencia = em
						.find(Contrareferencia.class, new Integer(
								idContrareferencia));
				...
				if (contrareferencia != null) {
					...
					String URL_SERVLET_LOGO = urlPath + "ServletImageLogo?id="
							+ contrareferencia.getCompania().getId()
							+ "&type=EMPRESA";

					String URL_SERVLET_IMAGE_ANULADA = "";
					if (contrareferencia.getEstado().equals("A")) {
					...
					parameters.put("ID_CONTRAREFERENCIA", new Integer(
							idContrareferencia));
					...
					String rutaReporte = urlPath
							+ "resources/report/historia_clinica/contrareferenciaHistoriaClinica.jasper";
