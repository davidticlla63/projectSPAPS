...
@WebServlet("/ReporteTransferenciaHC")
public class ReporteTransferenciaHC extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = -8780159590949416665L;
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			try {
				...
				String idTransferencia = request
						.getParameter("idTransferencia");
				// String pPathLogo = request.getParameter("pLogo");
				Transferencia transferencia = em.find(Transferencia.class,
						new Integer(idTransferencia));
				...
				if (transferencia != null) {
					...
					String URL_SERVLET_LOGO = urlPath + "ServletImageLogo?id="
							+ transferencia.getCompania().getId()
							+ "&type=EMPRESA";

					String URL_SERVLET_IMAGE_ANULADA = "";
					if (transferencia.getEstado().equals("A")) {
					...
					parameters.put("ID_TRANSFERENCIA", new Integer(
							idTransferencia));
					...
					String rutaReporte = urlPath
							+ "resources/report/historia_clinica/transferenciaHistoriaClinica.jasper";
