...
@FacesConverter("cargoTrabajoConverter")
public class CargoTrabajoConverter implements Converter {
	...
	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		if (value != null && value.trim().length() > 0) {
			try {
				 ...
				 * url.split("/");
				 */
				List<CargoTrabajo> pacientes = new ArrayList<CargoTrabajo>();
				pacientes = PacienteController.cargoTrabajos;
				CargoTrabajo paciente = getObjectFromList(pacientes,
	...
	public String getAsString(FacesContext fc, UIComponent uic, Object object) {
		if (object != null) {
			return String.valueOf(((CargoTrabajo) object).getId());
		} else {
			return null;
		}
	}
	...
	private CargoTrabajo getObjectFromList(final List<?> list,
			final Integer identifier) {
		for (final Object object : list) {
			final CargoTrabajo team = (CargoTrabajo) object;
