...
public class SucursalConverter implements Converter {
	...
	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		if (value != null && value.trim().length() > 0) {
			try {
				 ...
				 * url.split("/");
				 */
				List<Sucursal> pacientes = new ArrayList<Sucursal>();
				pacientes = DesgOrdenServicioController.sucursales;
				Sucursal paciente = getObjectFromList(pacientes,
	...
	private Sucursal getObjectFromList(final List<?> list,
			final Integer identifier) {
		for (final Object object : list) {
			final Sucursal team = (Sucursal) object;
