...
public class CodigoControl7 {
	...
	public String obtenerCodigoControl2(Date fechaFactura,
			String numeroFactura,String numeroAutorizacion,String llaveControl,
			double totalBolivianos, String nitCi) {
		try {
			...
			setLlaveDosificacion(llaveControl);
	...
	public String obtener() {
		...
		numFechaTransaccion = numFechaTransaccion
				+ ver.obtener(numFechaTransaccion);
		numMonto = numMonto + ver.obtener(numMonto);
		// numLlaveDosificacion = numLlaveDosificacion +
		// ver.obtener(numLlaveDosificacion);
		...
		long sumatoria = Long.parseLong(numFactura) + Long.parseLong(numNitci)
				+ Long.parseLong(numFechaTransaccion)
				+ Long.parseLong(numMonto);
		...
		// 5 digitos
		String digitos = sumaStr.substring(sumaStr.length() - 5,
				sumaStr.length());
		...
		String ctres = this.getLlaveDosificacion().substring(uno + dos,
				uno + dos + tres);
		String ccuatro = this.getLlaveDosificacion().substring(
				uno + dos + tres, uno + dos + tres + cuatro);
		String ccinco = this.getLlaveDosificacion().substring(
		...
		for (int u = 0; u < cifrado.length(); u++) {
			if (num == 0) {
				/*
				 * numero0 = this.obtieneAscii(cifrado.toCharArray()[u]) +
				 * numero0;
				 */
